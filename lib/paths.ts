...
export const SEC_CACHE = join(CACHE_DIR, 'sec-cache');
export const USASPENDING_RECIPIENT_CACHE = join(CACHE_DIR, 'usaspending-recipient-cache');
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  const entries: Record<string, string> = {
    ...
    LEGISLATORS_CACHE, OPENSECRETS_CACHE, USASPENDING_CACHE, SEC_CACHE,
    USASPENDING_RECIPIENT_CACHE, PIPE_DIR, SKILLS_DIR,
